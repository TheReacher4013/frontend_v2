...
const ExpenseCategories = () => {
  ...
  const loadCategories = async () => {
    try {
      ...
      setCategories((data.categories || []).map(c => ({ id: c.id, name: c.name, description: c.description || "" })));
  ...
  const handleSubmit = async () => {
    if (!formData.name) { alert("Expense Category Name is required"); return; }
    try {
      if (formData.id) { await api.updateExpenseCategory(formData.id, { name:formData.name, description:formData.description }); }
      else             { await api.createExpenseCategory({ name:formData.name, description:formData.description }); }
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.deleteExpenseCategory(id); await loadCategories(); }
