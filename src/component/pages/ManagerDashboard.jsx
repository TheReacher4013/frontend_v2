...
export default function ManagerDashboard() {
  ...
  const [overview,  setOverview]  = useState({ total_leads:0, pending_followups:0 });
  const [callStats, setCallStats] = useState({ total_calls:0, total_duration:"0H 0M" });
  const [pieLive,   setPieLive]   = useState([]);
  const [barLive,   setBarLive]   = useState([]);
  ...
  const loadDashboard = async () => {
    try {
      ...
      if (statusData) setPieLive(statusData.map((s,i) => ({ name:s.status, value:Number(s.count), color:COLORS[i%COLORS.length] })));
      if (perDay)     setBarLive(perDay.map(d => ({ date:d.date?.substring(5), calls:Number(d.count) })));
    } catch(e) { console.error(e); }
  };

  const pieData = pieLive.length ? pieLive : [
    { name: "New", value: 0, color: "#3f6212" },
  ];

  const barData = barLive.length ? barLive : [];
...
/>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6 mb-8">
        ...
        {[
          ...
          { label: "Active Campaigns", val: overview.total_leads || "0", icon: <FaBullhorn />, color: "bg-[#14b8a6]" },
          { label: "Total Follow Up", val: overview.pending_followups || "0", icon: <FaCalendarCheck />, color: "bg-[#22c55e]" },
          { label: "Call Made", val: callStats.total_calls || "0", icon: <FaPhone />, color: "bg-[#ea580c]" },
          { label: "Total Duration", val: callStats.total_duration || "0H 0M", icon: <FaClock />, color: "bg-[#f87171]" }
