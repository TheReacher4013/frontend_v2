...
function EditProfileModal({ initialAvatar, onClose, onSave }) {
  ...
  const [fullName, setFullName] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.name || "Manager"; } catch { return "Manager"; } });
  const [email, setEmail] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.email || "manager@company.com"; } catch { return "manager@company.com"; } });
  ...
  return (
    <ModalShell onClose={onClose}>
      ...
      <div className="overflow-y-auto max-h-[70vh] space-y-4 pr-1">
        ...
        <div className="flex flex-col items-center gap-1 mb-2">
          ...
          <div className="relative cursor-pointer group" onClick={() => editFileRef.current.click()}>
            {editAvatar ? (
              ...
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-blue-800 to-blue-600 flex items-center justify-center border-4 border-white shadow-lg">
        ...
        <div>
          ...
          <input value={fullName}
            ...
            className={`w-full border rounded-lg mt-1 p-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition ${errors.fullName ? "border-red-400" : "border-gray-300"}`}
        ...
        <div>
          ...
          <input type="email" value={email}
            ...
            className={`w-full border rounded-lg mt-1 p-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition ${errors.email ? "border-red-400" : "border-gray-300"}`}
        ...
        <div>
          ...
          <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)}
            className="w-full border border-gray-300 rounded-lg mt-1 p-2.5 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition"
            ...
            <div>
              ...
              <div className="relative mt-1">
                <input type={showOld ? "text" : "password"} value={oldPass}
                  ...
                  className={`w-full border rounded-lg p-2.5 text-sm outline-none focus:border-blue-500 pr-10 transition ${errors.oldPass ? "border-red-400" : "border-gray-300"}`}
            ...
            <div>
              ...
              <div className="relative mt-1">
                <input type={showNew ? "text" : "password"} value={newPass}
                  ...
                  className="w-full border border-gray-300 rounded-lg p-2.5 text-sm outline-none focus:border-blue-500 pr-10 transition"
      ...
      <div className="flex gap-3 mt-5">
        ...
        <button onClick={handleSave} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-all shadow-md">Save Changes</button>
      </div>
    </ModalShell>
  );
}
...
export function ProfileModal({ onClose }) {
  ...
  const [name, setName] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.name || "Manager"; } catch { return "Manager"; } });
  const [email, setEmail] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.email || "manager@company.com"; } catch { return "manager@company.com"; } });
    ...
    <ModalShell onClose={onClose}>
      ...
      <div className="flex flex-col items-center mb-5">
        ...
        <div className="relative cursor-pointer group mb-3" onClick={() => fileInputRef.current.click()}>
          {avatar ? (
            ...
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-blue-800 to-blue-600 flex items-center justify-center border-4 border-white shadow-lg">
        ...
        <p className="text-sm text-gray-500">{role === "manager" ? "Manager" : role}</p>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <button onClick={() => setEditOpen(true)}
          className="flex-1 flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white py-2.5 rounded-lg text-sm font-medium transition-all shadow-md">
          <FaEdit /> Edit Profile
        </button>
      </div>
    </ModalShell>
