...
function EditProfileModal({ initialAvatar, onClose, onSave }) {
  ...
  const [fullName, setFullName] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.name || "Admin"; } catch { return "Admin"; } });
  const [email, setEmail] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.email || "admin@company.com"; } catch { return "admin@company.com"; } });
...
export function ProfileModal({ onClose }) {
  ...
  const [name, setName]   = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.name || "Admin"; } catch { return "Admin"; } });
  const [email, setEmail] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.email || "admin@company.com"; } catch { return "admin@company.com"; } });
