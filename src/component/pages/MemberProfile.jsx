...
function EditProfileModal({ initialAvatar, onClose, onSave }) {
  ...
  const [email, setEmail] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.email || "member@company.com"; } catch { return "member@company.com"; } });
...
export function ProfileModal({ onClose }) {
  ...
  const [name, setName] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.name || "Member"; } catch { return "Member"; } });
  const [email, setEmail] = useState(() => { try { return JSON.parse(localStorage.getItem("user"))?.email || "member@company.com"; } catch { return "member@company.com"; } });
      ...
      <div className="flex flex-col items-center mb-5">
        ...
        <div className="relative cursor-pointer group mb-3" onClick={() => fileInputRef.current.click()}>
          ...
          <button onClick={(e) => { e.stopPropagation(); fileInputRef.current.click(); }}
            className="absolute bottom-0 right-0 w-7 h-7 bg-indigo-600 hover:bg-indigo-700 rounded-full flex items-center justify-center border-2 border-white shadow transition-colors">
