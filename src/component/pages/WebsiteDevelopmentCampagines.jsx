...
const defaultFormData = {
  referenceNumber: "MEP_5708413",
  leadStatus: "",
  firstName: "Dario",
  lastName: "Beatty",
  company: "Reichel-Schiller",
  email: "toney73@example.org",
  website: "kris.org",
  phone: "+17183709640",
  notes: "",
  companyName: "",
...
function WebsiteDevelopmentCampagines() {
  ...
  useEffect(() => {
    ...
    if (leadId) {
      api.getNotes(leadId).then(data => {
        const noteList = data.notes || data || [];
        setNotesList(noteList.map(n => ({
          id:   n.id,
          text: n.note,
          time: n.created_at?.substring(0,10) || new Date().toLocaleDateString()
        })));
      }).catch(console.error);
    }
  }, []);
  ...
  useEffect(() => {
    // Load lead from API if leadId is passed via state
    const leadId = location.state?.leadId;
    if (leadId) {
  ...
  const handleSave = () => {
    ...
    if (leadId) {
      ...
      api.updateLead(leadId, {
        ...
        status:  formData.leadStatus?.toLowerCase().replace(" ", "_") || undefined,
      }).catch(console.error);
    }
    ...
    <div className="p-4 md:p-6 bg-gray-100 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-3 mb-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-semibold">Website Development Campaign (MEP_5708413)</h1>
            <p className="text-xs md:text-sm text-gray-500">Dashboard - Call Manager - Website Development Campaign</p>
          </div>
        </div>
        <div className="flex gap-2 flex-shrink-0">
          <button className="bg-yellow-400 hover:bg-yellow-500 px-3 md:px-4 py-2 rounded text-white text-sm transition-colors">
            ← Previous Lead
          </button>
          <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="text-sm space-y-2 text-gray-600">
            <p>Lead Number : <b>5 / 23</b></p>
            <p>Last Actioner : <b>Admin</b></p>
            <p>Follow Up : <b>Marielle Rosenbaum</b></p>
            <p>Salesman Booking : <b>Lilla Wintheiser</b></p>
          </div>
          <hr className="my-4" />
              ...
              {isEditing && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input className={inputCls()} value={formData.referenceNumber} onChange={e => handleChange("referenceNumber", e.target.value)} />
                  </div>
                  <div>
                ...
                <table className="w-full border min-w-[500px]">
                  ...
                  <tbody>
                    <tr>
                      ...
                      <td className="p-2 border text-sm">28-02-2026 04:31 am</td>
                      <td className="p-2 border text-sm">Admin</td>
                    </tr>
                    <tr>
                      <td className="p-2 border text-sm">08 M, 12 S</td>
                      <td className="p-2 border text-sm">27-02-2026 10:59 am</td>
                      <td className="p-2 border text-sm">Admin</td>
                    </tr>
                  </tbody>
                </table>
        ...
        <div className="bg-white p-4 rounded shadow">
          ...
          <div className="text-sm text-gray-600 space-y-4">
            <p>A new call started by Admin<br /><span className="text-xs text-gray-400">28-02-2026 04:31 am</span></p>
            <p>A new call started by Admin<br /><span className="text-xs text-gray-400">27-02-2026 10:59 am</span></p>
            <p>Follow up added by Admin for staff member<br /><span className="text-xs text-gray-400">Marielle Rosenbaum</span></p>
          </div>
        </div>
