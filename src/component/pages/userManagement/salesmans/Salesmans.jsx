...
function Salesman() {
  ...
  useEffect(() => { loadSalesmans(); }, []);

  const loadSalesmans = async () => {
    try {
      const data = await api.getSalesmans();
      setSalesmanList((data.salesmans||[]).map(s => ({
        id:           s.id,
        name:         s.name,
        email:        s.email||"",
        phone:        s.phone||"",
        status:       s.status,
        address:      s.address||"",
        profileImage: s.profile_image || getAvatarUrl(s.name)
      })));
  ...
  const handleDelete = async (id) => {
    ...
    try { await api.deleteSalesman(id); await loadSalesmans(); }
