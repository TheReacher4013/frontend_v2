...
function SalesmanBookings() {
  ...
  const loadBookings = async () => {
    try {
      const [bData, sData] = await Promise.all([api.getAllBookings(), api.getSalesmans()]);
      ...
      setBookings(bList);
      const smOpts = (sData.salesmans||[]).map(s => ({ value:s.name, label:s.name }));
      setSalesmanOptions(smOpts);
      const camps = [...new Set(bList.map(b => b.campaign).filter(c => c !== "---"))];
      setCampaignOptions(camps.map(c => ({ value:c, label:c })));
  ...
  const handleConfirmDelete = async () => {
    try { await api.deleteBooking(selectedId); await loadBookings(); }
